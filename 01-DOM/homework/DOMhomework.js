...
function displayToDos() {
  // Tu código acá:
  toDoContainer = document.getElementById("toDoContainter");
  toDoContainer.innerHTML = "";
  let llamarBuild = buildToDos(toDoItems);
  for (let i = 0; i < llamarBuild.length; i++) {
    toDoContainer.appendChild(llamarBuild[i])
  }
}
...
function addToDo() {
  // Tu código acá:
  let imput = document.querySelector("#toDoImput");
  let newToDo = new ToDo (imput.value);
...
// Tu código acá: 
let addBoton = document.getElementById("addButton")
addBoton.addEventListener("click",addToDo);
